#!/usr/bin/env python3
"""Führt den instrumentierten Ersatzdatensatz ``costed_split_soft_n10`` aus.

Liest das Plan-Manifest ``results/costed_split_soft_n10_manifest.csv`` und
startet die geplanten Runs via ``fagan run --config <cfg> --run-id <rid>``.
Pro Run wird echte LLM-Tokenusage/Laufzeit nach ``runs/<run_id>/llm_usage.csv``
geloggt (automatisch durch den Provider bei dry_run=false).

Sicherheitsregeln:
- Das ursprüngliche Manifest wird NIE überschrieben. Der Status wird in eine
  separate Datei ``results/costed_split_soft_n10_manifest_status.csv``
  geschrieben.
...
STATUS_OUT = _PROJECT_ROOT / "results" / "costed_split_soft_n10_manifest_status.csv"
...
def _load_manifest() -> List[Dict[str, str]]:
    if not MANIFEST.exists():
        raise SystemExit(f"[costed_run] Manifest fehlt: {MANIFEST}")
...
def main() -> int:
    ...
    print(f"[costed_run] OPENAI_API_KEY found: {str(have_key).lower()}")

    if not have_key:
        print("[costed_run] OPENAI_API_KEY nicht gesetzt -> es werden KEINE Runs gestartet.")
        print("[costed_run] Geplante Befehle:")
    ...
    for r in rows:
        ...
        if run_dir.exists():
            ...
            skipped += 1
            print(f"[costed_run] SKIP (existiert): {run_id}")
            status_rows.append(out)
            continue
    ...
    print(f"[costed_run] gestartet={started}, skipped_exists={skipped}, "
          f"planned_no_api_key={planned}, failed={failed}")
    print(f"[costed_run] Status -> {STATUS_OUT} (Original-Manifest unverändert)")
    return 0
